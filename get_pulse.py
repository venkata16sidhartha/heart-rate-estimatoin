...
val=0
def user_detail():
    os.system('python user_details.py')
a=user_detail()
filepath = 'temp.txt'
a=[]
...
class getPulseApp(object):
    ...
    def __init__(self, args):
        ...
        self.processor = findFaceGetPulse(bpm_limits=[10, 160],
                                          data_spike_limit=2500.,
                                          face_detector_smoothness=10.)

        # Init parameters for the heart rate data plot
        self.bpm_plot = False
        self.plot_title = "Data display - intial input signal (top) and PSD (bottom)"
       

        # Maps keystrokes to specified methods
        #(A GUI window must have focus for these to work)
        self.key_controls = {"s": self.toggle_search,
                             "p": self.toggle_display_plot,
                             "c": self.toggle_cam,
                             "w": self.write_csv}
